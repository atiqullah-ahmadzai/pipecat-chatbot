...
class ChatbotClient {
  ...
  setupEventListeners() {
    this.connectBtn.addEventListener('click', (event) => {
      ...
      const websiteId = button.getAttribute('data-id');
      if (websiteId) {
          this.connect(websiteId);
      } else {
          console.warn("No data-id found on button.");
      }
  });
  ...
  log(message) {
    ...
    document.querySelector("#chat .card-body").scrollTop = document.querySelector("#chat .card-body").scrollHeight;
    console.log(message);
  }
